...
import AuroraPreview from './AuroraPreview.vue'
...
function buildHtml(data: PortfolioData, mainHtml: string, styles: string): string {
  ...
  const fontsHref =
    'https://fonts.googleapis.com/css2?family=Noto+Sans+SC:wght@300;400;500;700;800;900&family=Manrope:wght@400;500;600;700;800&display=swap'
  return `<!DOCTYPE html>
<html lang="zh-CN">
<head>
<meta charset="UTF-8">
...
/**
 * 把极光模板的当前数据导出为单文件 HTML 字符串。
 * AuroraPreview 已内联全部作品弹窗（初始隐藏，由 runtime 切换 is-open），
 * 故只需渲染一次组件，取 DOM 序列化，配合主文档样式表与原生 JS runtime 拼装成独立 HTML。
 */
export async function exportAuroraHtml(data: PortfolioData): Promise<string> {
  const host = document.createElement('div')
  host.style.cssText = 'position:fixed;left:-99999px;top:0;width:1200px;visibility:hidden;'
  document.body.appendChild(host)
  const app = createApp(AuroraPreview, { data })
  app.mount(host)
  await nextTick()
  await nextTick()

...
(function(){
  // 导航栏滚动样式
  var navbar = document.querySelector('.p-navbar');
  if (navbar) {
  ...
  document.querySelectorAll('.p-modal').forEach(function(m){
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape') {
      document.querySelectorAll('.p-modal.is-open').forEach(function(m){ closeModal(m); });
    }
  });

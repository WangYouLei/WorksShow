...
(function(){
  // 导航栏滚动样式
  var navbar = document.querySelector('.b-navbar');
  if (navbar) {
  ...
  document.querySelectorAll('.b-modal').forEach(function(m){
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape') {
      document.querySelectorAll('.b-modal.is-open').forEach(function(m){ closeModal(m); });
    }
  });

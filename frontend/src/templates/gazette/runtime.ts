...
(function(){
  // 导航栏滚动样式
  var navbar = document.querySelector('.g-navbar');
  if (navbar) {
  ...
  document.querySelectorAll('.g-modal').forEach(function(m){
  ...
  document.addEventListener('keydown', function(e){
    if (e.key === 'Escape') {
      document.querySelectorAll('.g-modal.is-open').forEach(function(m){ closeModal(m); });
    }
  });
